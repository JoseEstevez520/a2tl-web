# UIDL vs Markdown vs HTML — Benchmark Results
...
| Example | MD (bytes) | MD (tokens) | UIDL (bytes) | UIDL (tokens) | HTML (bytes) | HTML (tokens) | UIDL vs MD |
|---------|------------|-------------|--------------|----------------|--------------|----------------|------------|
...
UIDL is **11% smaller** than equivalent Markdown on average. The original README claimed UIDL cost 13% more tokens than Markdown — this was based on a single example with a shorter-than-realistic Markdown baseline.

When Markdown is written realistically (tables with alignment rows, headers, bold formatting, blockquotes), UIDL's terse syntax consistently comes in smaller.

## What UIDL adds that Markdown cannot

| Example | UIDL-exclusive features |
|---------|------------------------|
...
Every UIDL example produces a standalone HTML page with:
...
UIDL achieves the same visual output as the full HTML while using 89% fewer tokens.

## Summary

```
Markdown:  ~385 tokens  →  Plain text (no charts, no styling, no interactivity)
UIDL:      ~343 tokens  →  Full interactive page (charts, cards, themes, responsive)
HTML:    ~3,054 tokens  →  Full interactive page (same output as UIDL)
```

UIDL is the only format that is both compact (smaller than Markdown) and rich (same output as HTML). It achieves this by separating content specification from presentation — the AI specifies *what* to show, and a local renderer handles *how*.
